...
do
	...
	do
		...
		awk -v num1=$num1 -v num2=$num2 -v num3=$num3 -v num4="$num4"'{if ($num4 == "NA"){ {cnt[$num1"_"$num2"_"$num3"_"$num4]+=1}END{for (x in cnt){print x,cnt[x]}}' "$ArchaeType/Diatoms/$file" | sort > hash.temp.txt
